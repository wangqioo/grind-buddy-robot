...
from core.utils.voice_latency import mark_voice_latency
    ...
    async def handle_voice_stop(self, conn, asr_audio_task: List[bytes]):
        ...
        try:
            mark_voice_latency(
                conn,
                "voice_stop",
                detail=f"frames={len(asr_audio_task)} format={conn.audio_format}",
            )
            ...
            mark_voice_latency(
                conn,
                "asr_done",
                detail=f"asr_time={total_time:.3f}s text_len={len(content_for_length_check)}",
            )
