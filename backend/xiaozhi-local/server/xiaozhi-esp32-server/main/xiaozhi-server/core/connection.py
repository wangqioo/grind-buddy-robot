...
from core.utils.voice_latency import mark_voice_latency, mark_voice_latency_once
    ...
    def chat(self, query, depth=0):
        ...
        try:
            ...
            mark_voice_latency(
                self,
                "llm_request_start",
                detail=f"depth={depth} function_call={self.intent_type == 'function_call' and functions is not None}",
            )
        ...
        for response in llm_responses:
            ...
            if self.intent_type == "function_call" and functions is not None:
                ...
                if not tool_call_flag and content_arguments.startswith("<tool_call>"):
                    ...
                    tool_call_flag = True
                    mark_voice_latency_once(self, "llm_tool_call_start")
            ...
            if emotion_flag and content is not None and content.strip():
                mark_voice_latency_once(
                    self,
                    "llm_first_token",
                    detail=f"token_len={len(content)}",
                )
                asyncio.run_coroutine_threadsafe(
