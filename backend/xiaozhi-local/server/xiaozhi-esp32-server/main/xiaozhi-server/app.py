...
async def main():
    ...
    if runtime_profile.config_source == "local":
        logger.bind(tag=TAG).info(
            "OTA接口是\t\t{}",
            display_http_url(runtime_profile, detected_local_ip).replace(
                "/mcp/vision/explain", "/xiaozhi/ota/"
            ),
        )
    logger.bind(tag=TAG).info(
        "视觉分析接口是\t{}",
        display_http_url(runtime_profile, detected_local_ip),
    )
    ...
    logger.bind(tag=TAG).info(
        "Websocket地址是\t{}",
        display_websocket_url(runtime_profile, detected_local_ip),
    )
