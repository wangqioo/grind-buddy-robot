...
- ESP32-S3 主控：负责屏幕表情、麦克风、扬声器、Wi-Fi 和小智兼容语音链路。
...
- ESP32-S3 可以通过 OTA 和 WebSocket 连接本地小智兼容后台。
...
| 交互层 | ESP32-S3 | 设备状态、屏幕表情、音频、Wi-Fi、小智会话控制。 |
...
本地后台负责 AI 运行时能力，避免黑客松演示完全依赖远端业务服务器。
...
需要复现本地开发环境时，请使用 `backend/xiaozhi-local/` 下的示例配置和本地初始化脚本。
