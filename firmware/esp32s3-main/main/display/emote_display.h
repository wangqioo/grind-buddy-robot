...
private:
    ...
    void TryStartEmoteGenPreview();
    void PlayStandaloneEmote(const void* data, size_t size, uint8_t fps);
    void PlayEmoteGenAssetByIndex(int asset_index);
    bool PlayEmoteGenAssetByName(const std::string& filename);
    virtual bool Lock(int timeout_ms = 0) override;
    virtual void Unlock() override;

    std::unique_ptr<EmoteEngine> engine_;
    mmap_assets_handle_t emote_gen_assets_ = nullptr;
    std::map<std::string, int> emote_gen_asset_index_by_name_;
    bool emote_gen_runtime_active_ = false;
