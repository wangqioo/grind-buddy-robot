...
void WifiBoard::TryWifiConnect() {
    ...
    if (!have_ssid && HasDefaultWifiCredentials()) {
        auto credentials = GetDefaultWifiCredentials();
        ESP_LOGI(TAG, "No persisted WiFi credentials; using built-in default SSID %s", credentials.ssid);
        ssid_manager.AddSsid(credentials.ssid, credentials.password);
        have_ssid = true;
    }
