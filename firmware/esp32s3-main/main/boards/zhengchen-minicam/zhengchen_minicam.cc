...
private:
    ...
    void UpdateStateOutput() {
        if constexpr (STATE_OUTPUT_GPIO == GPIO_NUM_NC) {
            return;
        } else {
    ...
    void InitializeLcdDisplay() {
        ...
        display_ = new emote::EmoteDisplay(panel, panel_io, DISPLAY_WIDTH_1, DISPLAY_HEIGHT_1);
#else
    ...
    void InitializeCamera() {
        ...
        // Camera SCCB shares the audio codec I2C bus (I2C_NUM_0, SDA=GPIO1, SCL=GPIO2),
        // so esp_video must reuse the already initialized bus instead of creating its own.
        ...
        esp_video_init_dvp_config_t dvp_config = {
            ...
            // Keep the 20 MHz XCLK that the original minicam firmware was verified with.
            .xclk_freq = 20000000,
        };
...
public:
    zhengchen_minicam()
        ...
        InitializeCamera();
        InitializeTools();
        StartGrindBuddyUartIntegration();
    }

    virtual Led* GetLed() override {
        if constexpr (BUILTIN_LED_GPIO == GPIO_NUM_NC) {
            static NoLed led;
            return &led;
        } else {
